...
float U32toF32(unsigned int u32) { return *((float *)(&u32)); }
...
static PF_Err ParamsSetup(PF_InData *in_data, PF_OutData *out_data, PF_ParamDef *params[], PF_LayerDef *output) {
    ...
    AEFX_CLR_STRUCT(def);
    PF_ADD_POPUP("Type",2,1,"Invert|Flip",Params::TYPE); // origin 1
...
static PF_Err Render(PF_InData *in_data, PF_OutData *out_data, PF_ParamDef *params[], PF_LayerDef *output) {
    ...
    if(width>=256&&height>=256) {
        ...
        if(input->width==width&&input->height==height)  {
            ...
            int type = params[Params::TYPE]->u.bd.value-1;
            
            bool isInit = metal->init([queue device],width,height,FileManager::resource(@"org.mizt.MFR",(type==0)?@"Invert.metallib":@"Flip.metallib"));
            if(isInit) {
                ...
                [metal->drawableTexture() getBytes:buffer bytesPerRow:width*4 fromRegion:MTLRegionMake2D(0,0,width,height) mipmapLevel:0];
                
                NSLog(@"%d,%lu %lu",tid,MFR::threads.size(),[MTLCreateSystemDefaultDevice() currentAllocatedSize]);
